...
async def get_emissions(product_type: str = Query(..., description="Product type to filter emission factors")):
    try:
        emissions = []
        # Fetch the document based on product_type
        doc = await emission_factors_collection.find_one({"productType": product_type})
        if not doc:
            raise HTTPException(status_code=404, detail="Emission factors not found for the specified product type")

        # Iterate over emission factors in the document
        for ef in doc.get('emissionFactors', []):
            # Convert document to EmissionFactor model
            emission = EmissionFactor(
                id=str(doc['_id']),
                name=ef.get('process', ''),
                unit=ef.get('basis', ''),
                value=ef.get('value', 0.0),
